-- Phoenix A hub (script final corrigido)
...
local function startFly()
    ...
    state.flyConn = RunService.RenderStepped:Connect(function()
        ...
        if bv and bv.Parent then bv.Velocity = targetVel end
...
local function stopNoclip()
    ...
    for part, canCollide in pairs(savedCollisions) do
        if part and part:IsA("BasePart") then part.CanCollide = canCollide end
    end
    savedCollisions = {}
end
...
-- Monta UI Movimento / Extras
local order = 1
createButton(movimentoFrame, "Toggle Fly", order, function() if state.fly then stopFly() else startFly() end end)
order = order + 1

createSlider(movimentoFrame, "Fly Speed", 60, 10, 300, order, function(val) state.flySpeed = val end)
order = order + 1
...
createButton(movimentoFrame, "Teleport", order, function() teleportToPlayer(tpBox.Text) end)
order = order + 1
...
createButton(extrasFrame, "Toggle Noclip", 1, function() if state.noclip then stopNoclip() else startNoclip() end end)
...
if humanoid then
    ...
    humanoid.JumpPower = state.jumpPowe
